...
def save_incoming_request(response):
    g.response_list.append(response)

def minute_stat(method_list):
    valid_responses = []
    current_time = g.start
    for responses in g.response_list:
...
def general_stat(method_list):
    ...
    for methods in method_list:
        method_avg = methods+"_hour_avg"
        method_count = methods+"_hour_count"
        general_stat_dict[method_avg] = average_request(g.reponse_list,methods)
        general_stat_dict[method_count] = number_of_method_request(g.response_list,methods)
